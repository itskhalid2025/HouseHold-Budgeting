...
- [ ] Define Goal model
- [ ] Define CustomCategory model (NEW)
- [ ] Define RecurringExpense model (NEW)
- [ ] Define Loan and LoanRepayment models (NEW)
- [ ] Define SplitExpense and SplitRepayment models (NEW)
...
3. Transaction
   ...
   - recurringExpenseId (FK, optional - link to recurring)
   - loanRepaymentId (FK, optional - link to loan repayment)
   - splitExpenseId (FK, optional - link to split)
   - createdAt, updatedAt
...
7. CustomCategory (NEW - User-created categories)
   - id (UUID)
   - householdId (foreign key)
   - name (e.g., "Maid Salary", "Gym Membership")
   - type (NEEDS, WANTS, SAVINGS)
   - parentCategory (optional, for grouping)
   - isActive (boolean, default true)
   - createdAt

8. RecurringExpense (NEW - Regular payments like maid, subscriptions)
   - id (UUID)
   - householdId (foreign key)
   - name (e.g., "Maid", "Netflix", "Gym")
   - amount (Decimal)
   - category, subcategory
   - frequency (enum: DAILY, WEEKLY, MONTHLY, YEARLY)
   - skipDates (JSON array of skipped dates)
   - expectedAmount (auto-calculated based on frequency)
   - actualAmount (sum of actual transactions)
   - isActive (boolean, default true)
   - startDate, endDate (optional)
   - createdAt, updatedAt

9. Loan (NEW - Money lent or borrowed)
   - id (UUID)
   - householdId (foreign key)
   - userId (who logged it)
   - type (enum: LENT, BORROWED)
   - personName (who you lent to / borrowed from)
   - principalAmount (original amount)
   - remainingAmount (what's left)
   - dueDate (optional)
   - notes (optional)
   - isSettled (boolean, default false)
   - createdAt, updatedAt
   - repayments (relation to LoanRepayment[])

10. LoanRepayment (NEW - Partial or full repayment)
    - id (UUID)
    - loanId (foreign key)
    - amount (Decimal)
    - date
    - method (cash, UPI, bank transfer, etc.)
    - note (optional)
    - createdAt

11. SplitExpense (NEW - Bill splitting with friends)
    - id (UUID)
    - householdId (foreign key)
    - transactionId (FK to original transaction)
    - totalAmount (full bill amount)
    - yourShare (your portion)
    - splits (JSON: [{person, amount, isPaid}])
    - isFullySettled (boolean)
    - createdAt, updatedAt

12. SplitRepayment (NEW - When someone pays back their share)
    - id (UUID)
    - splitExpenseId (foreign key)
    - personName
    - amount
    - date
    - method (optional)
