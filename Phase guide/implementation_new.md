...
Users can create personalized expense categories that don't fit predefined ones.

```javascript
// Example: User creates "Maid Salary" under NEEDS
{
  name: "Maid Salary",
  type: "NEEDS",
  parentCategory: "Household Services"
}
```
...
- Unique recurring expenses

---

### 2. Recurring Expenses

Track regular payments with skip date tracking for services like maid, gym, Netflix.
...
// Recurring Expense Record
{
  ...
  amount: 6000,
  frequency: "MONTHLY",
  skipDates: ["2024-01-18", "2024-01-19"],  // Maid didn't come these days
  expectedAmount: 6000,
  actualAmount: 5600  // Calculated based on skip dates
}
```

**Features:**
- Mark skip dates when service wasn't used
...
Track money lent to or borrowed from friends/family with repayment tracking.
...
Split restaurant bills or shared expenses with friends.
...
// Split Expense Example
{
  ...
  yourShare: 1000,
  splits: [
    { person: "John", amount: 1000, isPaid: true },
    { person: "Jane", amount: 1000, isPaid: false }
  ]
}
```
...
Show remaining balance after all expenses with AI suggestions (ON REQUEST ONLY).
...
[Ask AI for Savings Suggestions] ← User clicks
```

**Key Points:**
- AI suggestions are **ON REQUEST ONLY**
- User clicks button to get suggestions
- Never automatic or intrusive

---

## New Database Models

The following models have been added to support advanced features:

| Model | Purpose |
|-------|---------|
| `CustomCategory` | User-created expense categories |
| `RecurringExpense` | Regular payments with skip dates |
| `Loan` | Money lent or borrowed |
| `LoanRepayment` | Partial/full loan repayments |
| `SplitExpense` | Bill splits with friends |
| `SplitRepayment` | When friends pay back |

---

## AI Detection for New Features

The AI categorization agent now detects:

| User Input | AI Detects | Action |
|------------|-----------|--------|
| "Gave loan to John 10000" | Loan | Creates Loan record |
| "John paid back 4000" | Repayment | Links to existing Loan |
| "Restaurant with friends 3000" | Split | Suggests SplitExpense |
| "Maid didn't come today" | Skip | Marks skip date on RecurringExpense |
| "Paid maid salary 6000" | Recurring | Links to/creates RecurringExpense |
